...
namespace CommonBuilder;

use CommonBuilder\Contracts\AuthContract;
use CommonBuilder\Contracts\PageContract;
use CommonBuilder\Contracts\PageTranslationContract;
use CommonBuilder\Contracts\WebsiteManagerContract;
use CommonBuilder\Contracts\PageBuilderContract;
use CommonBuilder\Contracts\RouterContract;
use CommonBuilder\Contracts\ThemeContract;
use CommonBuilder\Modules\GrapesJS\PageRenderer;
use CommonBuilder\Repositories\UploadRepository;
use CommonBuilder\Core\DB;
...
class CommonBuilder
{
    ...
    public function loadTranslations($language)
    {
        ...
        $themeTranslationsFolder = phpb_config('theme.folder') . 'CommonBuilder.php/' . phpb_config('theme.active_theme') . '/translations';
    ...
    public function handlePublicRequest()
    {
        ...
        if (strpos(phpb_current_relative_url(), phpb_config('general.uploads_url') . 'CommonBuilder.php/') === 0) {
            $this->handleUploadedFileRequest();
            header("HTTP/1.1 404 Not Found");
            exit();
        }
        // if we are on the URL of a PHPageBuilder asset, return the asset
        if (strpos(phpb_current_relative_url(), phpb_config('general.assets_url') . 'CommonBuilder.php/') === 0) {
    ...
    public function handleUploadedFileRequest()
    {
        ...
        $serverFile = realpath(phpb_config('storage.uploads_folder') . 'CommonBuilder.php/' . $uploadedFile->server_file);
        // add backwards compatibility for files uploaded with PHPageBuilder <= v0.12.0, stored as /uploads/{id}.{extension}
        if (! $serverFile) $serverFile = realpath(phpb_config('storage.uploads_folder') . 'CommonBuilder.php/' . basename($uploadedFile->server_file));
        if (! $serverFile) {
            header("HTTP/1.1 404 Not Found");
            exit();
        }
